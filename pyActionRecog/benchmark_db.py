...
def parse_ucf_splits():
    ...
    path_class_ind = '/workspace/Hidden-Two-Stream/models/ucf101_split1_unsup_end/'
    class_ind = [x.strip().split() for x in open(path_class_ind + 'classInd.txt')]
    ...
    splits = []
    for i in xrange(1, 4):
        train_list = [line2rec(x) for x in open(path_class_ind+'trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open(path_class_ind+'testlist{:02d}.txt'.format(i))]
